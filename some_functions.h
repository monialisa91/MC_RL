...
vec getConf(string filename, double U) {
    char tmp_char;
    double tmp_double;
    string result = "";
...
mat LastHamiltonian(int L, int t, double U, string filename) {
    int size_matrix = L*L;
    vec lista(size_matrix);
    lista = getConf(filename, U);
    mat lattice(size_matrix, size_matrix);
    lattice.zeros();
    cout << size_matrix << " " << lista.n_rows << endl;
...
mat MC(mat initial_hamiltonian, int MC_steps, double beta, double cp) {
